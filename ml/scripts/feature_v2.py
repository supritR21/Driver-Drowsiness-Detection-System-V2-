...
class BaseFrameFeatures:
    ...
    def to_temporal_array(
        self,
        previous: Optional["BaseFrameFeatures"],
    ) -> np.ndarray:
        ...
        if previous is None:
            ...
            delta_ear = self.ear_mean - previous.ear_mean
            delta_mar = self.mar - previous.mar

            delta_yaw = self.yaw - previous.yaw
            delta_pitch = self.pitch - previous.pitch
            delta_roll = self.roll - previous.roll
...
class FaceFeatureExtractorV2:
    ...
    def _estimate_head_pose(
        ...
        image_points = np.asarray(
            [
                tuple(points[index])
                for index in self.HEAD_POSE_INDICES
            ],
            dtype=np.float64,
        )
        ...
        pitch, yaw, roll = [
            float(value)
            for value in euler_angles.reshape(-1)[:3]
        ]
